...
class RadonArray extends RadType{
  ...
  RadonArray getArray(int i) => RadonArray(concat(script, [OP.ARRAY_GET_ARRAY, i]));
  RadonBoolean getBoolean(int i) => RadonBoolean(concat(script, [OP.ARRAY_GET_BOOLEAN, i]));
  RadonBytes getBytes(int i) => RadonBytes(concat(script, [OP.ARRAY_GET_BYTES, i]));
  RadonFloat getFloat(int i) => RadonFloat(concat(script, [OP.ARRAY_GET_FLOAT, i]));
  RadonInteger getInteger(int i) => RadonInteger(concat(script, [OP.ARRAY_GET_INTEGER, i]));
  RadonMap getMap(int i) => RadonMap(concat(script, [OP.ARRAY_GET_MAP, i]));
  RadonString getString(int i) => RadonString(concat(script, [OP.ARRAY_GET_STRING, i]));
...
class RadonFloat extends RadType{
  ...
  RadonBoolean greaterThan(double i)  => RadonBoolean(concat(script, [OP.FLOAT_GREATER_THAN, i]));
  RadonBoolean lessThan(double i)  => RadonBoolean(concat(script, [OP.FLOAT_LESS_THAN, i]));
  RadonInteger modulo(int i)  => RadonInteger(concat(script, [OP.FLOAT_MODULO, i]));
  RadonFloat multiply(num i)  => RadonFloat(concat(script, [OP.FLOAT_MULTIPLY, i]));
  ...
  RadonFloat truncate(int i)  => RadonFloat(concat(script, [OP.FLOAT_TRUNCATE, i]));
}
class RadonInteger extends RadType{
  ...
  RadonBoolean greaterThan(int i) => RadonBoolean(concat(script, [OP.INTEGER_GREATER_THAN, i]));
  RadonBoolean lessThan(int i) => RadonBoolean(concat(script, [OP.INTEGER_LESS_THAN, i]));
  RadonInteger modulo(int i) => RadonInteger(concat(script, [OP.INTEGER_MODULO, i]));
  RadonInteger multiply(int i) => RadonInteger(concat(script, [OP.INTEGER_MULTIPLY, i]));
  RadonInteger negate()  => RadonInteger(concat(script, OP.INTEGER_NEGATE));
  RadonInteger power(int i) => RadonInteger(concat(script, [OP.INTEGER_POWER, i]));
}
...
class RadonMap extends RadType{
  ...
  RadonArray getArray(String key) => RadonArray(concat(script, [OP.MAP_GET_ARRAY, key]));
  RadonBoolean getBoolean(String key) => RadonBoolean(concat(script, [OP.MAP_GET_BOOLEAN, key]));
  RadonBytes getBytes(String key) => RadonBytes(concat(script, [OP.MAP_GET_BYTES, key]));
  RadonFloat getFloat(String key) => RadonFloat(concat(script, [OP.MAP_GET_FLOAT, key]));
  RadonInteger getInteger(String key) => RadonInteger(concat(script, [OP.MAP_GET_INTEGER, key]));
  RadonMap getMap(String key) => RadonMap(concat(script, [OP.MAP_GET_MAP, key]));
  RadonString getString(String key) => RadonString(concat(script, [OP.MAP_GET_STRING, key]));
  RadonArray keys()  => RadonArray(concat(script, OP.MAP_KEYS));
  RadonArray valuesAsArray()  => RadonArray(concat(script, OP.MAP_VALUES_AS_ARRAY));
}
class RadonString extends RadType{
  RadonString([List? script]) : super((script == null) ? [] : script);
  RadonBoolean asBoolean()  => RadonBoolean(concat(script, OP.STRING_AS_BOOLEAN));
  RadonFloat asFloat()  => RadonFloat(concat(script, OP.STRING_AS_FLOAT));
  RadonInteger asInteger()  => RadonInteger(concat(script, OP.STRING_AS_INTEGER));
  RadonInteger length()  => RadonInteger(concat(script, OP.STRING_LENGTH));
  RadonBoolean match(Map<String, dynamic> key, bool state)  => RadonBoolean(concat(script, [OP.STRING_MATCH, key, state]));
  RadonArray parseJSONArray()  => RadonArray(concat(script, OP.STRING_PARSE_JSON_ARRAY));
  RadonMap parseJSONMap()  => RadonMap(concat([], OP.STRING_PARSE_JSON_MAP));
  RadonString toLowerCase()  => RadonString(concat(script, OP.STRING_TO_LOWER_CASE));
  RadonString toUpperCase()  => RadonString(concat(script, OP.STRING_TO_UPPER_CASE));
}
...
class Witnet {
  ...
  static RADRetrieve Source(String url,
     dynamic script,
  ...
  static RADRetrieve GraphQLSource(String url,
    ...
    return RADRetrieve(
      ...
      headers: List<StringPair>
        .from(headers.entries.toList().map((e) =>
          StringPair(left: e.key, right: e.value)))
    );
  }
