...
class CodecAES extends Codec<Object, String> {
  ...
  CodecAES(
      {required Uint8List key, required Uint8List iv, required AesMode mode}) {
...
CodecAES getCodecAES(String password,
    {required Uint8List iv, required Uint8List salt}) {
  return CodecAES(
      key: keyFromPassword(password: password, salt: salt),
      iv: iv,
      mode: AesMode.cbc);
}
...
Uint8List generateIV() {
  ...
  return Uint8List.fromList(
      List<int>.generate(IV_LENGTH, (index) => random.nextInt(256)));
}

Uint8List generateSalt() {
  var random = Random.secure();
  return Uint8List.fromList(
      List<int>.generate(SALT_LENGTH, (index) => random.nextInt(256)));
}
