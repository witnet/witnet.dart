...
Point addSamePoint(Point point, BigInt modNum, BigInt a) {
  var ru = positiveMod(
      (BigInt.from(3) * point.x.pow(2) + a) * inverseMulti(BigInt.two * point.y, modNum),
      modNum);
...
Point addDiffPoint(Point point1, Point point2, BigInt modNum) {
  var ru = positiveMod((point2.y - point1.y) * inverseMulti(point2.x - point1.x, modNum), modNum);
  var x3 = positiveMod(ru.pow(2) - point1.x - point2.x, modNum);
  var y3 = positiveMod(ru * (point1.x - x3) - point1.y, modNum);
  return Point(x3, y3);
}

Point  getPointByBigInt(
    BigInt n, BigInt p, BigInt a, Point pointG) {
